...
possible_edges_between_communities = 0
...
for i in range(len(G.nodes)):
    for j in range(i + 1, len(G.nodes)):
        ...
        if labels[i] != labels[j]:
            possible_edges_between_communities += 1

# 计算边缘密度
if possible_edges_between_communities > 0:
    edge_density = edges_between_communities / possible_edges_between_communities
else:
    edge_density = 0
