...
from src.maxcut_ppo_bihyb_model import ActorNet, CriticNet, GraphEncoder
from utils.utils import print_args
from utils.tfboard_helper import TensorboardUtil
from normalized_cut_ppo_bihyb_eval import evaluate
...
class ItemsContainer:
    ...
    def update(self, idx, reward=None, inp_lower_matrix=None, edge_candidates=edge_candidates, greedy=None, done=None,
               ori_greedy=None):
...
class PPO:
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, state_values, dist_entropy = self.policy.evaluate(lower_left_matrices, edge_candidates,
                                                                        old_actions)
...
def main(args):
    ...
    # create environment
    maxcut_env = Env(node = args.node,graph_type=args.graph_type,low_stage=args.low_stage)
    ...
    for i_episode in range(1, args.max_episodes + 1):
        ...
        graph_index = (i_episode - 1)  % len(tuples_train)
        inp_lower_matrix, edge_candidates,ori_greedy = tuples_train[graph_index]
            #print(graph_index)
        greedy = ori_greedy
        items_batch.append(0, inp_lower_matrix, edge_candidates,greedy,False,ori_greedy)
            # print('items',items_batch)

        for t in range(args.max_timesteps):
            timestep += 1

            #backup_inp_lower_matrix = items_batch.inp_lower_matrix
            # Running policy_old:
            with torch.no_grad():
            ...
            def step_func_feeder(batch_size):
                ...
                yield graph_index,batch_inp_lower_matrix[0], action_batch[:, 0],batch_greedy[0],False

            step_list = [maxcut_env.step(*x) for x in step_func_feeder(args.batch_size)]
            for b, item in enumerate(step_list):
                reward, inp_lower_matrix, edge_candidates,greedy,done = item

                if t == args.max_timesteps - 1:
                    done = True
                items_batch.update(b, reward=reward, inp_lower_matrix=inp_lower_matrix, edge_candidates=edge_candidates,
                                    done=done)
        ...
        if i_episode % args.log_interval == 0:
            ...
            print('episode:{},avg length:{},critic mse:{},reward:{},time per episode:{}'.format(i_episode,avg_length,critic_loss,running_reward,avg_time))

            running_reward = 0
            avg_length = 0
            critic_loss = []
        if args.low_stage == 'spc':
            file_name = f'./ncut_with_spc/PPO_{args.solver_type}_node_{args.node}_type_{args.graph_type}' \
                        f'_beam{args.search_size}_opt{best_test_optimum:.4f}.pt'
        elif args.low_stage == 'gnn':
            file_name = f'./ncut_with_gnn/PPO_{args.solver_type}_node_{args.node}_type_{args.graph_type}' \
                        f'_beam{args.search_size}_opt{best_test_optimum:.4f}.pt'
        torch.save(ppo.policy.state_dict(), file_name)
        print('save to',file_name)
        # testing
        # if i_episode % args.test_interval == 0:
        # # if i_episode is args.max_episodes-1:
        # #     print("save")
        #     with torch.no_grad():
        ...
        #         # run testing
        #         test_dict = evaluate(ppo.policy, maxcut_env, tuples_test, args.max_timesteps, args.search_size, mp_pool)
        #         print('test_dict["optimum"]["mean"]',test_dict["optimum"]["mean"])
        ...
        #     if test_dict["total_ours"]<best_ncut_mean:
        #         print('best_test_optimum',best_test_optimum)
        #         best_test_optimum = test_dict["optimum"]["mean"]
        #         print('test_dict["total_ours"]:',test_dict["total_ours"])
        #         print('best_ncut_mean',best_ncut_mean)
        #         best_ncut_mean=test_dict["total_ours"]
        #         file_name = f'./ncut/PPO_{args.solver_type}_node_{args.node}_type_{args.graph_type}' \
        #                     f'_beam{args.search_size}_opt{best_test_optimum:.4f}.pt'
        #         torch.save(ppo.policy.state_dict(), file_name)
...
def parse_arguments():
    ...
    parser.add_argument('--node',default=80,type=int,help="number of nodes")
    parser.add_argument('--graph_type',default='er',type=str,help="type of the graph")
    parser.add_argument('--low_stage',default=None,type=str,help="method chosen as low stage")
